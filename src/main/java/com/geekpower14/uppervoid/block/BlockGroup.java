...
public class BlockGroup {
    ...
    public BlockGroup(JsonArray group) {
        ...
        this.blockFine = new SimpleBlock(Material.matchMaterial(blockFineData[0]), Integer.parseInt(blockFineData[1]));

        String[] blockWarningData = group.get(1).getAsString().split(", ");
        this.blockWarning = new SimpleBlock(Material.matchMaterial(blockWarningData[0]), Integer.parseInt(blockWarningData[1]));

        String[] blockCriticalData = group.get(2).getAsString().split(", ");
        this.blockCritical = new SimpleBlock(Material.matchMaterial(blockCriticalData[0]), Integer.parseInt(blockCriticalData[1]));
    }
    ...
    public boolean isThis(Block block) {
        ...
        if (this.is(block, this.blockFine))
            ...
            return true;
        else return this.is(block, this.blockCritical);
    }
